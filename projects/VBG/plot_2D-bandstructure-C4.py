...
if __name__ == '__main__':
    data_path = 'data/k_space-lossy_material-2.csv'
    ...
    df_sample["fake_factor (1)"] = df_sample["S_air_prop (1)"]
    # 筛选k的成分
    # df_sample = df_sample[np.abs(df_sample["k"]) < 0.26]
    # 指定用于构造网格的参数以及目标数据列
    param_keys = ["k", "delta_shrink (nm)"]
    ...
    from projects.VBG.plot_thickband import main

    main(data_path)

    from projects.VBG.plot_band_Qfactors import main
