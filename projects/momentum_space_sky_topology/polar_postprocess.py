...
def plot_lines_along_isofreq(samples_list, labels=None):
    ...
    """
    if labels is None: labels = [f"path {i+1}" for i in range(len(samples_list))]
    fig, axs = plt.subplots(3, 1, figsize=(6.2, 6.5), sharex=True)
    for samp, lab in zip(samples_list, labels):
        s = samp['s']
        if 'phi' in samp:
            axs[0].plot(s, np.mod(samp['phi'], np.pi), lw=1.5, label=lab)
        elif 'phi_full' in samp:
            axs[0].plot(s, np.mod(samp['phi_full'], np.pi), lw=1.5, label=lab)
        if 'chi' in samp:
            axs[1].plot(s, samp['chi'], lw=1.5, label=lab)
        if 'Q' in samp:
            axs[2].plot(s, samp['Q'], lw=1.5, label=lab)
    axs[0].set_ylabel('phi (mod π)')
    axs[1].set_ylabel('tanchi')
    axs[2].set_ylabel('Q'); axs[2].set_xlabel('arc length s')
