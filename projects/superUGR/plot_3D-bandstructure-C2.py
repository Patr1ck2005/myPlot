...
if __name__ == '__main__':
    data_path = 'data/4000Q-diff_fill_factor-diff_kx-7eigens.csv'
    ...
    # # 筛选k的成分
    # df_sample = df_sample[df_sample["k"] <= 0.090]
    # 指定用于构造网格的参数以及目标数据列
    param_keys = ["k", "fill", "delta_factor"]
    ...
    zs = []
    colors = []
    for i, m1 in enumerate(new_coords['k']):
        for j, m2 in enumerate(new_coords['fill']):
    ...
    im = ax.imshow(ulog.T, origin='lower',
              extent=(new_coords['k'][0], new_coords['k'][-1], new_coords['fill'][0], new_coords['fill'][-1]),
    ...
    plotter = TwoDimParaSpacePlotter(config=config, data_path=data_path, coordinate_keys={'x': 'k', 'y': 'fill'})
    plotter.load_data()
