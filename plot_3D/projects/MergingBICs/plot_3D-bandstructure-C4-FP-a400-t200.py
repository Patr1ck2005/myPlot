from plot_3D.advance_plot_styles.polar_plot import *
...
from plot_3D.core.data_postprocess.momentum_space_toolkits import complete_C4_polarization, geom_complete
...
if __name__ == '__main__':
    ...
    from utils import extract_basic_analysis_fields, plot_advanced_surface
    ...
    fig, ax = plt.subplots(figsize=(3 / 2, 3 / 2))
    ax.imshow(qlog_f.T, extent=(m1f.min(), m1f.max(), m2f.min(), m2f.max()), origin='lower', cmap='hot', aspect='auto')
    plt.savefig('temp.svg')
    plt.show()

    fig, ax = plt.subplots(figsize=(3 / 2, 3 / 2))
    ax.imshow(phi_f.T, extent=(m1f.min(), m1f.max(), m2f.min(), m2f.max()), origin='lower', cmap='twilight', aspect='auto')
    plt.savefig('temp.svg')
    plt.show()
