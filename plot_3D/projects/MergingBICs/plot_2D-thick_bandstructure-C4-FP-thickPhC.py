...
if __name__ == '__main__':
    ...
    def recognize_sp(phi_arr, kx_arr, ky_arr):
        ...
        for phi, kx, ky in zip(phi_arr, kx_arr, ky_arr):
            if np.isclose(ky, 0):
                if np.isclose(phi, np.pi/2, atol=1e-2):
                    sp_polar.append(1)
                else:
                    sp_polar.append(0)
            elif np.isclose(ky, kx):
                if np.isclose(phi, np.pi/4, atol=1e-2):
                    sp_polar.append(1)
                else:
                    sp_polar.append(0)
            else:
    ...
    print(df_sample["phi (rad)"])
    df_sample["sp_polar_show"] = recognize_sp(df_sample["phi (rad)"], df_sample["m1"], df_sample["m1"])
    print(df_sample["sp_polar_show"])
    # # 筛选m1<0.1的成分
