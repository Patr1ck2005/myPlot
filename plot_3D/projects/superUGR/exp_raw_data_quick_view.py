...
def load_and_visualize_ftir_spectra(directory, prefix, start_num, end_num, angles,
                                    file_ext='.dpt', wavenumber_col=0, transmittance_col=1,
    ...
    - transmittance_col: int, 透射率列索引，默认 1。
    - save_plots: bool, 是否保存图像，默认 False。
    - plot_dir: str or None, 保存图像的目录，如果 save_plots=True，则必须提供。

    返回:
    - data_dict: dict, 键为角度，值为 (wavenumbers, transmittances) 的元组。
    """
    # 检查角度列表长度是否匹配文件数量
    ...
    # 初始化数据存储
    data_dict = {}
    all_transmittances = []  # 用于heatmap的2D数组
    ...
    for i, num in enumerate(range(start_num, end_num + 1)):
        ...
        # 读取数据：假设两列，制表符分隔
        data = np.loadtxt(filepath, usecols=(wavenumber_col, transmittance_col))
        wavenumbers = data[:, 0]
        transmittances = data[:, 1]

        data_dict[angle] = (wavenumbers, transmittances)
    ...
    all_transmittances = np.array(all_transmittances)
    ...
    for i, (angle, (wavenumbers, transmittances)) in enumerate(data_dict.items()):
        plt.plot(wavenumbers, transmittances, label=f'Angle: {angle}°', color=color_list[i])
    plt.xlabel('Wavenumber (cm⁻¹)')
    plt.ylabel('Transmittance')
    plt.title('FTIR Transmittance Spectra at Different Angles')
    plt.legend()
    plt.grid(True)
    if save_plots:
    ...
    # 纵轴波数，横轴角度(0-15)
    plt.imshow(
        all_transmittances.T, aspect='auto',
