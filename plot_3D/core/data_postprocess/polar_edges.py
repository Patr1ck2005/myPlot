...
def show_phi_families_split(xgrid, ygrid, phi, *, overlay='phi', figsize=(5,4), **plot_kw):
    """
    便捷接口：创建 fig/ax 并调用 plot_phi_families_split，返回 (fig, ax, res)。
    """
    fig, ax = plt.subplots(figsize=figsize)
    res = plot_phi_families_split(ax, xgrid, ygrid, phi, overlay=overlay, **plot_kw)
    plt.tight_layout()
    return fig, ax, res
