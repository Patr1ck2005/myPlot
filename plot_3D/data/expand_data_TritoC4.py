...
for _, row in df.iterrows():
    ...
    anti_coords = generate_anti_symmetry_coords(m1s, m2s)
    for i, anti_coord in enumerate(anti_coords):
        ...
        # function_df[2] -= 2*(theta-np.pi/4)
        function_df['phi (rad)'] = (np.pi / 2 - phi)
        expanded_dict['function_df'].append(function_df)
    expanded_dict['coordinates'].append((m2, m1))
    function_df = row.drop(['m1', 'm2']).values
    expanded_dict['function_df'].append(function_df)
...
for _, row in expanded_df.iterrows():
    ...
    m2s = row['m2']
    # # 生成 C4 对称性的坐标
    expanded_coords = generate_c4_symmetry_coords(m1s, m2s)
    # 为每个生成的坐标创建新的行并保存函数值
